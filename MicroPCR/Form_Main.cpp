...
BEGIN_MESSAGE_MAP(CForm_Main, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_PWM_APPLY, &CForm_Main::OnBnClickedButtonPwmApply)
END_MESSAGE_MAP()
...
void CForm_Main::readProtocol(CString path)
{
	...
	for (int i = 0; i<m_totalActionNumber; i++)
	{
		...
		{
			if (actions[i - 1].Label == "SHOT") continue;

			if ((actions[i].Temp > (double)_wtof(actions[i - 1].Label)) || (actions[i].Temp < 1))
			{
...
void CForm_Main::OnBnClickedButtonPwmApply()
{
