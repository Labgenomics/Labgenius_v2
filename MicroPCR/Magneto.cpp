...
int CMagneto::runTask()
{
	...
	if (ab.args[0] == 0 || ab.args[0] == 1 || ab.args[0] == 2) {
		nRtn = FAS_GetCommandPos(comPortNo, ab.args[0], &tempPos);
	...
	MotorPos motorPos = { targetPos, cmdPos, driverErrCnt };
...
{
	if (FAS_ServoAlarmReset(comPortNo, MotorType::CHAMBER) != FMM_OK)
		driverErrCnt++;

	if (FAS_ServoAlarmReset(comPortNo, MotorType::FILTER) != FMM_OK)
		driverErrCnt++;

	if (FAS_ServoAlarmReset(comPortNo, MotorType::PUMPING) != FMM_OK)
		driverErrCnt++;
}
