...
void CMicroPCRDlg::Serialize(CArchive& ar)
{
	...
	if (ar.IsStoring())
	{
		ar << m_cMaxActions << m_cTimeOut << m_cArrivalDelta << m_cGraphYMin << m_cGraphYMax << m_cIntegralMax << m_cCompensation << (&m_form_main)->m_cWG_PWM << (&m_form_main)->m_cRed_PWM << (&m_form_main)->m_cGreen_PWM << (&m_form_main)->m_cBlue_PWM;
	}
...
void CMicroPCRDlg::GetDebugTime(int tag, int command)
{
	DWORD d_diffTime = 0;
	CString t_temp = L"";
	...
	{
	case 0:
		d_preTime = timeGetTime();
		break;
	
	case 1:
		d_curTime = timeGetTime();

		d_diffTime = d_curTime - d_preTime;
		t_temp.Format(L"%d\t%d\t%d\t%d\n", tag, d_preTime, d_curTime, d_diffTime);
		debug_test.WriteString(t_temp);

		printf("%d\t%d\t%d\t%d\n", tag, d_preTime, d_curTime, d_diffTime);
		break;

	case 2:
		debug_test.WriteString(L"\n");
		break;
	}
}
...
LRESULT CMicroPCRDlg::OnmmTimer(WPARAM wParam, LPARAM lParam)
{
	...
	tx.led_wg_pwm = (&m_form_main)->m_cWG_PWM;
	tx.led_r_pwm = (&m_form_main)->m_cRed_PWM;
	tx.led_g_pwm = (&m_form_main)->m_cGreen_PWM;
	tx.led_b_pwm = (&m_form_main)->m_cBlue_PWM;
...
void CMicroPCRDlg::OnBnClickedButtonPcrRecord()
{
	if (!isRecording)
	{
		...
		CString fileName, fileName2, fileName3, d_test;
		...
		d_test = time.Format(L"./Record/debugs%Y%m%d-%H%M-%S.txt");
		...
		debug_test.Open(d_test, CStdioFile::modeCreate | CStdioFile::modeWrite);
